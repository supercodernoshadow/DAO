...
describe('DAO', () => {
  ...
  beforeEach(async () => {
    ...
    token = await Token.deploy('Dapp University', 'DAPP', '2000000')

    // Send tokens to investors - each one gets 10%
    transaction = await token.connect(deployer).transfer(investor1.address, tokens(1200000))
  ...
  describe('Deployment', () => {
    ...
    it('sends tokens to the DAO', async () => {
  ...
  describe('Proposal creation', () => {
    ...
    describe('Failure', () => {
      ...
      it('reject non-investor', async () => {
        await expect(dao.connect(user).createProposal('Proposal 1', ether(100), recipient.address, 'test')).to.be.reverted
      })

      it('reject non-descriptive proposals', async () => {
        await expect(dao.connect(investor1).createProposal('Proposal 1', ether(100), recipient.address, '')).to.be.reverted
      })
    })
  })
  ...
  describe('Voting', () => {
    ...
    describe('Success', () => {
      ...
      it('descreases vote count', async () => {
